...
/*
    ...
    FE > registration 선택 >
      http://localhost:8080/oauth2/authorization/#{registrationId}?redirect_uri=${redirectUrl} 백엔드 요청
       ( registrationId : 플랫폼 이름, redirectUrl : 소셜로그인 완료 후 FE로 보내야하는 주소값 )
    ...
    > OAuth2AuthenticationRequestBasedOnCookieRepository - saveAuthorizationRequest 메소드 호출
         ( 쿠키에 요청 정보, FE Redirect URL 저장 > 쿠키사용은 플랫폼과 통신을 여러번 하는 동안 데이터 유지에 사용 )
    ...
    loadUser, handler 부분은 사이트에 맞게 수정해야함.
 */
